...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.normalize_layers = torch.nn.ModuleList(
            [
                Normalize(
                    self.configs.enc_in if configs.features == 'M' else 1,  # 根据features调整
                    affine=True,
    ...
    def classify(self, x_enc):
        ...
        # 简化特征融合：只使用最高分辨率的特征
        # 最高分辨率特征包含最丰富的信息，同时计算量最小
        main_features = enc_out_list[0]  # 最高分辨率特征
        B_N, T, C = main_features.shape
        B = x_enc[0].shape[0]
        N = x_enc[0].shape[2]

        # 简单有效的特征提取
        features = main_features.reshape(B, N, T, C)
        # 时间维度最大池化（保留重要特征）
        features = features.max(dim=2)[0]  # [B, N, C]
        # 通道维度平均池化
        features = features.mean(dim=1)  # [B, C]

        # 分类输出
        logits = self.classifier(features.unsqueeze(-1))
